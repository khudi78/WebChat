...
const Message = ({ message }) => {
  ...
  console.log("msg_detail   ", message.senderId, authUser._id, fromMe);
  console.log("an id  ", message._id);
  // Fetch groups using the custom hook
  const { loading, groups } = useGetGroups();

  const [profilePic, setProfilePic] = useState("");
  const { loading: loadingConversations, conversations } =
    useGetConversations();

  useEffect(() => {
    if (fromMe) {
      // If the message is from the current user, use their profile picture
      console.log("avatar ", authUser.profilePic);
      setProfilePic(authUser.profilePic);
      
    } else {
      const member = message.receiverId;
      const targetConv = conversations.find(
        (conv) => conv._id === message.receiverId
	  );
	  console.log("targetConv ", targetConv);
	  setProfilePic(
		targetConv ? targetConv.profilePic : selectedConversation?.profilePic
      );

      console.log("recieve", member.profilePic);
    }
    if (!loading && groups.length > 0) {
      ...
      const group = groups.find(
        (g) =>
          g._id ===
          (message.groupId ? message.groupId : message.newMessage?.groupId)
      );
      ...
      if (group) {
        ...
        const member = group.members.find(
          (member) =>
            member._id ===
            (message.senderId ? message.senderId : message.newMessage?.senderId)
        );
        if (member) {
  ...
  const isFormattedTimeValid =
    formattedTime && formattedTime !== "Invalid date";
  const isNewMessageTimeValid =
    newMessageTime && newMessageTime !== "Invalid date";

  // Use formattedTime or newMessageTime for footer
  const footerTime = isFormattedTimeValid
    ? formattedTime
    : isNewMessageTimeValid
    ? newMessageTime
    : "No time available";

  console.log("message ", message);
  console.log("NewMessage: ", message.message);
  console.log("ProfilePic: ", authUser.profilePic); // Log the selected profile picture
  ...
  return (
    <div className={`chat ${chatClassName}`}>
      ...
      <div
        className={`chat-bubble text-white ${bubbleBgColor} ${shakeClass} pb-2 whitespace-pre-wrap max-w-xs break-words`}
      >
        {displayedMessage}
      </div>
